...
const Header = () => {
  return (
    <header className="relative z-[999]">
      ...
      ></motion.div>
      <nav className="fixed left-1/2 top-[0.15rem] flex h-12 -translate-x-1/2 items-center justify-between py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
        <ul className="flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-base text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-5">
          {links.map((link) => (
            ...
            >
              <Link
                className="flex w-full items-center justify-center p-3 text-inherit transition-all hover:text-gray-950"
                href={link.hash}
              >
                {link.name}
              </Link>
